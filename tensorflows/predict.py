...
import tensorflows.readers
import tensorflows.latent_factor_model
...
def predict():
    ...
    df_train, df_test = tensorflows.latent_factor_model.get_data()
    ...
    iter_train = tensorflows.readers.ShuffleIterator([df_train["user"],
                                          df_train["item"],
                                          df_train["rate"]],
                                         batch_size=batch_size)

    # Sequentially generate one-epoch batches, for testing
    iter_test = tensorflows.readers.OneEpochIterator([df_test["user"],
                                          df_test["item"],
                                          df_test["rate"]],
                                         batch_size=-1)
    ...
    with tf.Session() as sess:
        ...
        for i in range(max_epochs * samples_per_batch):
            ...
            _, pred_batch = sess.run([train_op, infer], feed_dict={user_batch: users,
                                                                   item_batch: items,
                                                                   rate_batch: rates})
            pred_batch = tensorflows.latent_factor_model.clip(pred_batch)
